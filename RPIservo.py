...
class ServoCtrl(threading.Thread):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.initPos = load_json(self.servo_json_path)["servo_angles"]
        self.limit_angles_min = [(self.initPos[i] - 85) for i in range(16)]
        self.limit_angles_max = [(self.initPos[i] + 85) for i in range(16)]
    ...
    def adjust_init_angle(self, servo_index, dir, offset = 2):
        angle_temp = max(self.limit_angles_min[servo_index], min(self.limit_angles_max[servo_index], self.initPos[servo_index] + offset*dir))
        self.set_angle(servo_index, angle_temp)
        self.initPos[servo_index] = angle_temp

    def persist_Servos_init(self , servo_index):
        json_data = load_json(self.servo_json_path)
        json_data["servo_angles"][servo_index] = self.initPos[servo_index]
        save_json(json_data, self.servo_json_path)

    def init_single_servo(self, servo_index):
        self.set_angle(servo_index, 90)
        self.initPos[servo_index] = 90
